...
- **サーマルフュージョンオプションの追加**
  - スマホ用サーマルカメラを利用したサーマルフュージョンオプションを友人のgb7k氏に作成いただきました。
  - 詳細および必要ファイルは[こちら](https://github.com/Qoo622/3D-print/tree/master/Yume-Vision/YUME_v1/DayDream)より。
- **その他未実装の将来構想**
  - 本体の微改造を伴う全長の短縮
  - アイピース接続オプション
...
| YUME_v1.2_Front | レンズおよび本体の固定用のパーツです | 1個 |
...
| YUME_v1.2_Spacer | Frontパーツを組み込んだ際にメイン基板と干渉する場合、間に挟んで利用してください | 1個 |
| --- | ▼ 用途別に選択して準備するもの　|
...
| M3x22mmキャップボルト | 偏光板調光機能を持たせる際に14mmキャップボルトと入れ替えて使います | 4本 |
| YUME_v1.1_lens-protect-holder | 対物側の防弾処理をするために使います | キット分 |
| YUME_v1.1_Upper_20mm | 20mmレールに直接マウントする際に使います | 1つ |
| YUME_v1.1_20mm-adapter | 20mmレールに直接マウントする際に使います | 1つ |
| M4ナット&M4x28mm皿ネジ | 20mmレールに直接マウントする際に使います | 1組 |
| YUME_v1.1_Connecter-cover_ext | 外部電源化する際に使います | キット分 |
...
> [!NOTE]
> メイン基板と干渉する場合はSpacerパーツを印刷して間に挟むようにしてみてください。
...
### 4. 20mmダイレクトマウント
ライフルのトップレールなどに直接マウントする際に使います。ホロサイト等の４５mm高に合わせてあります。
- **【用意するもの】**
  - YUME_v1.1_Upper_20mm
  - YUME_v1.1_20mm-adapeter
  - 残りのキット一式
  - M4x28mm皿ネジ+M4ナット
- **【組み立て方】**
  - 通常通りキットを組み、Upperのみ上記パーツに入れ替える
  - 20mm-adapterをあてがい、レールを挟むようにしてM4ネジで固定する
- **【留意事項】**
  - 配置の関係で録画機能を使うと映像は上下逆さまになります。編集ソフト等で回転させてください。
...
| Date | Update |
| :--- | :--- |
| 2024/05/07 | サーマルフュージョンオプション 『Day Dream』 を公開しました。
| 2024/04/08 | サポート必要量を減らしネジ穴の強度を上げたFront v1.2と組み込み用のスペーサーを公開しました |
| 2024/04/04 | ２０mmレールマウント用パーツと外部電源用のコネクターカバーを公開しました |
